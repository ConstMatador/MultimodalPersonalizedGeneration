...
class UserPosterDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(poster_path):
            poster_path = os.path.join(self.poster_dir, f"{movie_id}.png")
        if not os.path.exists(poster_path):
            raise FileNotFoundError(f"Poster not found for movieId={movie_id}")
...
class UserAdapter(nn.Module):
    def __init__(self, in_dim=256, hidden=1024, out_dim=4096, n_user_tokens=1, dropout=0.1):
        super().__init__()
...
# Core Model Wrapper
# -----------------------------
class FLUXPosterPersonalizer(nn.Module):
    def __init__(self, model_id="/root/TOS/ZhongzhengWang/model/FLUX.1-dev", device="cuda", n_user_tokens=4):
        super().__init__()
        self.device = device
        ...
        self.pipe_call_params = set(inspect.signature(self.pipe.__call__).parameters.keys())

    @torch.no_grad()
    def encode_prompt_with_neg(self, prompts, negative_prompts):
        """
        兼容不同 diffusers 版本的 FLUX encode_prompt 返回值。
        目标：稳定拿到
          - prompt_embeds
        ...
        # 某些版本支持 negative_prompt / negative_prompt_2
        sig = inspect.signature(self.pipe.encode_prompt)
        if "negative_prompt" in sig.parameters:
            encode_kwargs["negative_prompt"] = negative_prompts
        if "negative_prompt_2" in sig.parameters:
        ...
        if isinstance(encoded, tuple):
            ...
            # 或
            # (prompt_embeds, pooled_prompt_embeds, text_ids, negative_prompt_embeds, negative_pooled_prompt_embeds, ...)
            tensors = [x for x in encoded if torch.is_tensor(x)]

            if len(tensors) >= 1:
                prompt_embeds = tensors[0]
            if len(tensors) >= 2:
                pooled_prompt_embeds = tensors[1]

            # 如果 encode_prompt 直接返回了负向 embedding，就优先使用
            if len(tensors) >= 4:
                negative_prompt_embeds = tensors[2]
                negative_pooled_prompt_embeds = tensors[3]
        elif torch.is_tensor(encoded):
            prompt_embeds = encoded
        else:
            raise ValueError(f"Unexpected return type from encode_prompt: {type(encoded)}")

        if prompt_embeds is None:
            raise ValueError("Failed to get prompt_embeds from pipe.encode_prompt(...)")

        # 当前报错的关键：pooled_prompt_embeds 不能为空
        if pooled_prompt_embeds is None:
            raise ValueError(
                "Failed to get pooled_prompt_embeds from pipe.encode_prompt(...). "
                "Your current diffusers/FLUX version requires pooled_prompt_embeds when passing prompt_embeds."
            )
    ...
    def build_cond(self, prompts, user_emb_256, negative_prompts, use_user_tokens=True):
        ...
        if use_user_tokens:
            ...
            cond_embeds = torch.cat([user_tokens.to(prompt_embeds.dtype), prompt_embeds], dim=1)
            negative_cond_embeds = torch.cat(
                [torch.zeros_like(user_tokens, dtype=prompt_embeds.dtype), negative_prompt_embeds], dim=1
            )
        else:
    ...
    def generate_images(
        ...
        # 按当前 diffusers 版本支持情况，动态传负向参数
        if "negative_prompt_embeds" in self.pipe_call_params:
            pipe_kwargs["negative_prompt_embeds"] = negative_embeds
        if "negative_pooled_prompt_embeds" in self.pipe_call_params:
            pipe_kwargs["negative_pooled_prompt_embeds"] = negative_pooled_embeds
...
def main(cfg: TrainConfig, local_rank: int):
    ...
    try:
        ...
        model = FLUXPosterPersonalizer(model_id=cfg.model_id, device=str(device), n_user_tokens=cfg.n_user_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", type=str, default="/root/TOS/ZhongzhengWang/model/FLUX.1-dev")
